...
using static SeedsPleaseLite.ModSettings_SeedsPleaseLiteRedux;
...
public static class OptionsDrawUtility
{
	...
	public static void DrawListItem(Listing_Standard options, ThingDef def)
	{
		...
		if (selectedTab == Tab.seedless)
        {
            checkOn = seedlessCache.Contains(hash);
        }
        else
        {
            checkOn = false;
		...
		else if (hashCache.Contains(hash))
        {
